...
export class KaControl {
  ...
  valueChanged(value, old) {
    if (
      this.bindedToResource &&
      this.bindedResource.data &&
      this.checkNested(this.bindedResource.data, this.name.split("."))
    )
  ...
  bindToResource() {
    ...
    if (resource.schema) {
      const setSchemaBinding = () => {
        ...
        if (this.checkNested(resource.schema, this.name.split(".")))
          this.schema = eval(`resource.schema.${this.name}`);
        // Set observer for control's schema changes on resource
        if (this.resourceControlSchemaBinding)
          this.resourceControlSchemaBinding.dispose();
        this.resourceControlSchemaBinding = this.binding
          .expressionObserver(this, `bindedResource.schema.${this.name}`)
          .subscribe((value) => (this.schema = value));
      };
      setSchemaBinding();
      // Reset binding if resource schema gets entirely replaced
      this.resourceSchemaBinding = this.binding
        .expressionObserver(this, `bindedResource.schema`)
        .subscribe((value) => setSchemaBinding());
    } else if (this.schema) {
    ...
    if (resource.data) {
      ...
      const setDataBinding = () => {
        ...
        if (this.checkNested(resource.data, this.name.split(".")))
          this.value = eval(`resource.data.${this.name}`);
        // Set observer for control's data changes on resource
        if (this.resourceControlValueBinding)
          this.resourceControlValueBinding.dispose();
        this.resourceControlValueBinding = this.binding
          .expressionObserver(this, `bindedResource.data.${this.name}`)
          .subscribe((value) => {
      ...
      setDataBinding();
      // Reset binding if resource schema gets entirely replaced
      this.resourceValueBinding = this.binding
        .expressionObserver(this, `bindedResource.data`)
        .subscribe((value) => setDataBinding());
    }
