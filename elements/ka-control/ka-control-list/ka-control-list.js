...
export class KaControlsList {
  ...
  schemaChanged(schema) {
    ...
    if (!schema.datasource.model) {
      console.error(
        "ka-control-list: model missing from datasource!",
        schema.datasource
      );
      return;
    }
    if (!schema.datasource.model.schema) {
      console.error(
        "ka-control-list: schema missing from datasource model!",
        schema.datasource.model
      );
      return;
    }
    if (
      !Array.isArray(schema.datasource.model.schema) ||
      !schema.datasource.model.schema.length
    ) {
      console.error(
        "ka-control-list: datasource model schema must be an array of objects!",
        schema.datasource.model
      );
      return;
    }
